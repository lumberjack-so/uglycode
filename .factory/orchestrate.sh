...
# Usage: .factory/orchestrate.sh <sprint-number> [max-stages]
#
# Runs the full factory cycle:
# 1. Prepare context handoff
# 2. Print the Ralph Wiggum command to paste
...
SPRINT=${1:?"Usage: .factory/orchestrate.sh <sprint-number> [max-stages]"}
MAX_STAGES=${2:-3}
STAGE=1
...
EOF
    ...
    echo "  /ralph-wiggum:ralph-loop --max-iterations 15 --completion-promise \"DONE\" \"Read .llm/stage-context.md first, then .llm/todo.md. Work through each unchecked task top to bottom. For each task: read existing code and tests first, check .llm/exemplars/ for patterns, write failing test, implement minimally to pass, run full test suite, commit with conventional message, mark task complete in todo.md, update .llm/state.md. At HARD STOP tasks, stop and report. Do not skip tasks. Do not refactor beyond what the task requires. Output <promise>DONE</promise> when all tasks are complete.\""
    echo ""
    echo "  DIAL481/RALPH FORK:"
    echo "  /ralph:ralph-loop \"Read .llm/stage-context.md first, then .llm/todo.md. Work through each unchecked task top to bottom. For each task: read existing code and tests first, check .llm/exemplars/ for patterns, write failing test, implement minimally to pass, run full test suite, commit with conventional message, mark task complete in todo.md, update .llm/state.md. At HARD STOP tasks, stop and report. Do not skip tasks. Do not refactor beyond what the task requires. Output <promise>DONE</promise> when all tasks are complete.\" --max-iterations 15 --completion-promise \"DONE\""
    echo ""
